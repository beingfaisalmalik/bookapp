...
def index():
    ...
    message=Markup("""HEY THERE WELCOME TO MY WEBSITE SEARCH BOOKS AND READ REVIEWS """)
    session["books"]=[]
...
def bookpage(isbn):
    ...
    if request.method=="POST" and secondreview==None:
        ...
        db.commit()
    if request.method=="POST" and secondreview!=None:
        warning="ONE USER CAN GIVE ONLY ONE REVIEW ."
        ...
        if emailLogIn==None:                                  ## registration
            ...
            for i in range(len(data)):
                if data[i]["username"]==email:
                    log_in_message="USERNAME ALREADY EXIST "
            ...
            db.commit()
            log_in_message="YOU ARE REGISTERED PLEASE LOGIN TO CONTINUE."
        else:                                                 ## registration
            ...
            if data!=None:
                if data.username==emailLogIn and data.password==userPasswordLogIn:
                    ...
                    log_in_message="WRONG PASSWORD OR EMAIL TRY AGAIN!!."
            else:
                log_in_message="WRONG PASSWORD OR EMAIL TRY AGAIN!!."
    return render_template('login.html',log_in_message=log_in_message)
